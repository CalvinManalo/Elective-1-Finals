...
Route::middleware('guest')->group(function () {
    // allow optional role in the registration url (eg: /register/student)
    Route::get('register/{role?}', [RegisteredUserController::class, 'create'])
        ->name('register');

    // allow posting to /register/{role?} as well so forms that include role in the url
    // can submit directly to the controller
    Route::post('register/{role?}', [RegisteredUserController::class, 'store']);

    // allow optional role in the login url (eg: /login/student)
